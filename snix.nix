...
let
  ...
  # Post-build hook: automatically upload every build output into the snix
  # store so nar-bridge can serve it. Nix invokes this script after each
  # successful build with OUT_PATHS set to the space-separated output paths.
  uploadToSnixCache = pkgs.writeShellScript "upload-to-snix-cache" ''
    set -euo pipefail
    ...
    ${config.nix.package}/bin/nix store sign --recursive --key-file /var/cache-priv-key.pem $OUT_PATHS
...
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    nix.settings = lib.mkMerge [
      ...
      {
        # Automatically upload every build output into the snix store.
        post-build-hook = uploadToSnixCache;
      }
    ];
